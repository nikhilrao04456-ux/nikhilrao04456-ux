<h1 align="center">Hi, I'm Nikhil 👋</h1>

<h3 align="center">
AI/ML Engineer | Research Intern @ IIIT Nagpur
</h3>

<p align="center">
I build machine learning systems end to end —
from model training to production deployment
</p>

---

## 👨‍💻 What I Do

- 🚀 Build and deploy ML models into 
     production systems
- 🛰️ Research generalizable deep learning 
     for hyperspectral remote sensing
- 🌱 Develop AI solutions for real-world 
     agricultural problems
- 🏥 Ship full-stack ML web applications 
     on cloud infrastructure

---

## 🔧 Technical Skills

**ML & Deep Learning**
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat&logo=pytorch&logoColor=white)
![Scikit-learn](https://img.shields.io/badge/ScikitLearn-F7931E?style=flat&logo=scikit-learn&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=flat&logo=numpy&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat&logo=pandas&logoColor=white)
![OpenCV](https://img.shields.io/badge/OpenCV-5C3EE8?style=flat&logo=opencv&logoColor=white)

**Deployment & MLOps**
![Flask](https://img.shields.io/badge/Flask-000000?style=flat&logo=flask&logoColor=white)
![AWS EC2](https://img.shields.io/badge/AWS_EC2-232F3E?style=flat&logo=amazonaws&logoColor=white)
![REST API](https://img.shields.io/badge/REST_API-009688?style=flat)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)

**Frontend**
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)

---

## 🚀 Featured Projects

### 🏥 MedBuddyy — Full Stack ML Application
> End-to-end heart disease prediction system
- Built REST API backend with Flask
- Developed interactive frontend
- Deploying on AWS EC2
- **Stack:** Python, Flask, Scikit-learn, HTML/CSS/JS, AWS

---

### 🛰️ Hyperspectral Scene Generalization
> Generalizable deep learning for remote sensing
- Cross-scene classification without retraining
- Self-supervised masked spectral pretraining
- Spectral-spatial transformer architecture
- Uncertainty-aware domain adaptation
- **Datasets:** Houston, Pavia University, Salinas
- **Status:** 📄 Targeting CVIP 2025

---

### 🌱 Soil Property Prediction
> AI for precision agriculture
- Real field-collected hyperspectral data
- Predicts moisture content and Fe oxide levels
- PCA + spectral analysis pipeline
- **Status:** 📄 Targeting CVIP 2025

---

### 📊 Trader Sentiment Analysis
> Market behavior meets trader psychology
- Analyzed Fear & Greed Index vs market returns
- Built interactive visualizations and dashboards
- **Stack:** Python, Pandas, Matplotlib, Seaborn

---

### 💳 Credit Card Fraud Detection
> Tackling extreme class imbalance in finance
- Implemented SMOTE for minority oversampling
- Optimized for Recall and F1 over raw accuracy
- **Stack:** Python, Scikit-learn, Imbalanced-learn

---

## 📊 GitHub Stats

<p align="center">
<img src="https://github-readme-stats.vercel.app/api?username=nikhilrao04456-ux&show_icons=true&theme=tokyonight&hide_border=true" />
</p>

---

## 📫 Let's Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat&logo=linkedin&logoColor=white)]([YOUR_LINKEDIN](https://www.linkedin.com/in/nikhil-yadav-37281132a?utm_source=share_via&utm_content=profile&utm_medium=member_android))
[![Email](https://img.shields.io/badge/Gmail-D14836?style=flat&logo=gmail&logoColor=white)](mailto:nikhilrao044.56@gmail.com)

<!--
**nikhilrao04456-ux/nikhilrao04456-ux** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
